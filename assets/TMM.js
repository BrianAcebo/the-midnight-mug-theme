...
(() => {
			...
			window.tram = (function (a) {
				...
				var l = (function (a2, b2, c2) {
						...
						function g2(h2, i2) {
							function j2() {
								var a3 = new k2();
								return (
									e2(a3.init) && a3.init.apply(a3, arguments),
									a3
								);
							}
							function k2() {}
							i2 === c2 && ((i2 = h2), (h2 = Object)),
								(j2.Bare = k2);
							var l2,
								m2 = (f2[a2] = h2[a2]),
				...
				var J = (b.frame = (function () {
					...
					L = l(function (b2) {
						...
						function t2() {
							...
							for (a2 in this.props)
								(b3 = this.props[a2]),
									b3.active && c2.push(b3.string);
							(c2 = c2.join(',')),
								this.style !== c2 &&
									((this.style = c2),
									(this.el.style[G.transition.dom] = c2));
						}
						...
						function y2(a2, c2) {
							b2[a2] = function () {
								return this.children
									...
									: (this.el && c2.apply(this, arguments),
									  this);
							};
						}
						function A2(a2, b3) {
					...
					M = l(L, function (b2) {
						function c2(b3, c3) {
							var d2 =
								a.data(b3, q) || a.data(b3, q, new L.Bare());
					...
					N = l(function (a2) {
						...
						(a2.init = function (a3, b3, d3, e2) {
							...
							d3[2] && (g2 = d3[2]),
								...
								(this.unit =
									e2.unit || this.unit || U.defaultUnit),
								(this.angle =
									e2.angle || this.angle || U.defaultAngle),
						...
						(a2.init = function () {
							...
							(a2.transition = function (a3) {
								...
								for (c3 in this.current)
									d2[c3] =
										c3 in b3 ? b3[c3] : this.current[c3];
								(this.active = true),
									(this.nextStyle = this.style(d2));
							}),
							(a2.fallback = function (a3) {
								var b3 = this.values(a3);
								this.tween = new T({
	...
	// packages/shared/render/plugins/BaseSiteModules/tmm-lib.js
	var require_tmm_lib = __commonJS({
		'packages/shared/render/plugins/BaseSiteModules/tmm-lib.js'(
			exports,
			module
		) {
			...
			var modules = {};
			var primary = [];
			var secondary = window.tmm || [];
			var $ = window.jQuery;
			var $win = $(window);
			var $doc = $(document);
			var isFunction = $.isFunction;
			var _ = (tmm._ = require_underscore_custom());
			var tram = (tmm.tram = require_tram_min() && $.tram);
			var domready = false;
			var destroyed = false;
			tram.config.hideBackface = false;
			tram.config.keepInherited = true;
			tmm.define = function (name, factory, options) {
				if (modules[name]) {
			...
			tmm.require = function (name) {
				return modules[name];
			};
			function bindModule(module1) {
				if (tmm.env()) {
					isFunction(module1.design) &&
						$win.on('__tmm_design', module1.design);
					isFunction(module1.preview) &&
						$win.on('__tmm_preview', module1.preview);
				}
				isFunction(module1.destroy) &&
					$win.on('__tmm_destroy', module1.destroy);
			...
			function unbindModule(module1) {
				isFunction(module1.design) &&
					$win.off('__tmm_design', module1.design);
				isFunction(module1.preview) &&
					$win.off('__tmm_preview', module1.preview);
				isFunction(module1.destroy) &&
					$win.off('__tmm_destroy', module1.destroy);
				if (module1.ready && isFunction(module1.ready)) {
					removeReady(module1);
				}
			}
			...
			tmm.env = function (mode) {
				var designFlag = window.__tmm_design;
				...
				if (mode === 'slug') {
					return inApp && window.__tmm_slug;
				}
				if (mode === 'editor') {
					return window.tmmEditor;
				}
				if (mode === 'test') {
					return window.__tmm_test;
				}
				if (mode === 'frame') {
			...
			var touch = (tmm.env.touch =
				'ontouchstart' in window ||
				(window.DocumentTouch &&
					document instanceof window.DocumentTouch));
			var chrome = (tmm.env.chrome =
				/chrome/.test(userAgent) &&
				/Google/.test(navigator.vendor) &&
				parseInt(userAgent.match(/chrome\/(\d+)\./)[1], 10));
			var ios = (tmm.env.ios = /(ipod|iphone|ipad)/.test(userAgent));
			tmm.env.safari = /safari/.test(userAgent) && !chrome && !ios;
			var touchTarget;
			touch &&
				$doc.on('touchstart mousedown', function (evt) {
					touchTarget = evt.target;
				});
			tmm.validClick = touch
			...
			var resizeEvents = 'resize.tmm orientationchange.tmm load.tmm';
			var scrollEvents = 'scroll.tmm ' + resizeEvents;
			tmm.resize = eventProxy($win, resizeEvents);
			tmm.scroll = eventProxy($win, scrollEvents);
			tmm.redraw = eventProxy();
			...
			tmm.location = function (url) {
				window.location = url;
			};
			if (tmm.env()) {
				tmm.location = function () {};
			}
			tmm.ready = function () {
				domready = true;
				if (destroyed) {
			...
			tmm.destroy = function (options) {
				options = options || {};
				destroyed = true;
				$win.triggerHandler('__tmm_destroy');
			...
			$(tmm.ready);
			bindLoad();
			module.exports = window.tmm = tmm;
		},
	});

	// packages/shared/render/plugins/BaseSiteModules/tmm-brand.js
	var require_tmm_brand = __commonJS({
		'packages/shared/render/plugins/BaseSiteModules/tmm-brand.js'(
			exports,
			module
		) {
			'use strict';
			var tmm = require_tmm_lib();
			tmm.define(
				'brand',
				(module.exports = function ($) {
					var api = {};
					...
					var brandElement;
					api.ready = function () {
						var shouldBrand = $html.attr('data-tmm-status');
						var publishedDomain =
							$html.attr('data-tmm-domain') || '';
						if (
							/\.tmm\.io$/i.test(publishedDomain) &&
					...
					function createBadge() {
						var $brand = $('<a class="tmm-tmm-badge"></a>').attr(
							'href',
					...
					function ensureBrand() {
						...
						var match =
							found.length && found.get(0) === brandElement;
						var inEditor = tmm.env('editor');
						if (match) {
							if (inEditor) {
								found.remove();
							}
	...
	var require_tmm_focus_visible = __commonJS({
		'packages/shared/render/plugins/BaseSiteModules/tmm-focus-visible.js'(
			exports,
			module
		) {
				...
				(module.exports = function () {
					function applyFocusVisiblePolyfill(scope) {
						...
						function addFocusVisibleAttribute(el) {
							if (el.getAttribute('data-tmm-focus-visible')) {
								return;
							}
							el.setAttribute('data-tmm-focus-visible', 'true');
						}
						function removeFocusVisibleAttribute(el) {
							if (!el.getAttribute('data-tmm-focus-visible')) {
								return;
							}
							el.removeAttribute('data-tmm-focus-visible');
						}
	...
	// packages/shared/render/plugins/BaseSiteModules/tmm-focus.js
	var require_tmm_focus = __commonJS({
		'packages/shared/render/plugins/BaseSiteModules/tmm-focus.js'(
			exports,
			module
		) {
	...
	// packages/shared/render/plugins/BaseSiteModules/tmm-ix-events.js
	var require_tmm_ix_events = __commonJS({
		'packages/shared/render/plugins/BaseSiteModules/tmm-ix-events.js'(
			exports,
			module
		) {
			...
			api.triggers = {};
			api.types = {
				INTRO: 'tmm-ix-intro' + namespace,
				OUTRO: 'tmm-ix-outro' + namespace,
			};
			api.init = function () {
	...
	// packages/shared/render/plugins/BaseSiteModules/tmm-ix2-events.js
	var require_tmm_ix2_events = __commonJS({
		'packages/shared/render/plugins/BaseSiteModules/tmm-ix2-events.js'(
			exports,
			module
		) {
			'use strict';
			var IXEvents = require_tmm_ix_events();
	...
	var require_lib2 = __commonJS({
		'node_modules/redux/lib/index.js'(exports) {
			...
			var _bindActionCreators2 =
				_interopRequireDefault(_bindActionCreators);
			...
			function mergeIn4(a, path, b, c, d, e, f) {
				...
				if (rest.length) {
					nextVal = doMerge.call.apply(
						doMerge,
						[null, false, false, prevVal, b, c, d, e, f].concat(
							rest
						)
					);
				} else {
			...
			ixSession = (state = initialState2, action) => {
				switch (action.type) {
					case IX2_SESSION_INITIALIZED2: {
						const { hasBoundaryNodes, reducedMotion } =
							action.payload;
	...
	var require_hashSet = __commonJS({
		'node_modules/lodash/_hashSet.js'(exports, module) {
			...
			function hashSet(key, value) {
				...
				data[key] =
					nativeCreate && value === void 0 ? HASH_UNDEFINED : value;
				return this;
			}
					...
					if (
						...
						!(
							...
							(key == 'length' || // Node.js 0.10 has enumerable non-index properties on buffers.
								(isBuff &&
									(key == 'offset' || key == 'parent')) || // PhantomJS 2 has enumerable non-index properties on typed arrays.
								(isType &&
									(key == 'buffer' ||
										key == 'byteLength' ||
	...
	function createElementState(state, ref, refType, elementId, actionItem) {
		const refId =
			refType === PLAIN_OBJECT2
				? (0, import_timm3.getIn)(actionItem, [
						'config',
						'target',
						'objectId',
				  ])
	...
	var require_IX2Lottie = __commonJS({
		'packages/systems/ix2/plugins/IX2Lottie.js'(exports) {
			...
			var getPluginDuration3 = (element, actionItem) => {
				...
				return (
					parseFloat(element.getAttribute('data-default-duration')) *
					1e3
				);
			};
	...
	var require_IX2Spline = __commonJS({
		'packages/systems/ix2/plugins/IX2Spline.js'(exports) {
			...
			var queryContainerElement = (elementId) =>
				document.querySelector(`[data-tmm-id="${elementId}"]`);
			var getFrontendModule = () => window.tmm.require('spline');
			...
			var createPluginInstance3 = (element, actionItem) => {
				...
				return pluginElementId
					? queryContainerElement(pluginElementId)
					: null;
			};
	...
	var require_IX2Rive = __commonJS({
		'packages/systems/ix2/plugins/IX2Rive.js'(exports) {
			...
			var FIT_KEY = '--tmm-rive-fit';
			var ALIGNMENT_KEY = '--tmm-rive-alignment';
			var queryContainerElement = (elementId) =>
				document.querySelector(`[data-tmm-id="${elementId}"]`);
			var getFrontendModule = () => window.tmm.require('rive');
			var getPluginConfig2 = (actionItemConfig, key) =>
			...
			var renderPlugin2 = (
				...
				const StateMachineInputType =
					frontendModule.rive.StateMachineInputType;
				...
				function renderRive(riveInstance) {
					...
					function render() {
						const stateMachineInputs =
							riveInstance.stateMachineInputs(name);
						...
						if (FIT_KEY in inputs || ALIGNMENT_KEY in inputs) {
							...
							const nextFit =
								inputs[FIT_KEY] ?? currentLayout.fit;
							const nextAlignment =
								inputs[ALIGNMENT_KEY] ??
								currentLayout.alignment;
							if (
	...
	var require_plugins = __commonJS({
		'packages/systems/ix2/plugins/index.js'(exports) {
			...
			var _sharedconstants =
				(init_shared_constants(),
				__toCommonJS(shared_constants_exports));
	...
	var init_IX2VanillaPlugins = __esm({
		'packages/systems/ix2/shared/logic/IX2VanillaPlugins.ts'() {
			...
			pluginMethod = (methodName) => (actionTypeId) => {
				...
				if (!plugin) {
					throw new Error(
						`IX2 no plugin configured for: ${actionTypeId}`
					);
				}
	...
	function getInstanceOrigin(
		...
		if (isPluginType(actionTypeId)) {
			return getPluginOrigin(actionTypeId)(
	...
	function getDestinationValues({ element, actionItem, elementApi }) {
		if (isPluginType(actionItem.actionTypeId)) {
			return getPluginDestination(actionItem.actionTypeId)(
				actionItem.config
			);
		}
		switch (actionItem.actionTypeId) {
			...
			case STYLE_TEXT_COLOR: {
				...
				if (globalSwatchId && globalSwatchId.startsWith('--')) {
					...
					import_normalizeColor.normalizeColor)(value);
					return {
	...
	function parseColor({
		...
		return {
			...
			rValue: (0, import_defaultTo.default)(
				parseInt(matches[0], 10),
				255
			),
			// @ts-expect-error - TS2345 - Argument of type 'string | undefined' is not assignable to parameter of type 'string'.
			gValue: (0, import_defaultTo.default)(
				parseInt(matches[1], 10),
				255
			),
			// @ts-expect-error - TS2345 - Argument of type 'string | undefined' is not assignable to parameter of type 'string'.
			bValue: (0, import_defaultTo.default)(
	...
	function clearActionGroupStyles({ actionGroup, event, elementApi }) {
		...
		actionItems.forEach((actionItem) => {
			...
			if (isPluginType(actionTypeId)) {
				clearElement = (ref) =>
					clearPlugin(actionTypeId)(ref, actionItem);
			} else {
			...
			getAffectedElements({ config, event, elementApi }).forEach(
				clearElement
			);
		});
	}
	...
	function getActionListProgress(actionList, instance) {
		...
		actionItemGroups.forEach((group, index) => {
			...
			const carrierItem =
				actionItems[getMaxDurationItemIndex(actionItems)];
			const { config, actionTypeId } = carrierItem;
			if (instanceItem.id === carrierItem.id) {
				elapsedDuration = totalDuration + verboseTimeElapsed;
			}
			const duration =
				getRenderType(actionTypeId) === RENDER_GENERAL2
					? 0
					: config.duration;
	...
	var require_shared = __commonJS({
		'packages/systems/ix2/shared/index.js'(exports) {
			...
			var _IX2BrowserSupport = /* @__PURE__ */ _interop_require_wildcard(
				(init_IX2BrowserSupport(),
				__toCommonJS(IX2BrowserSupport_exports))
			);
			...
			var _IX2ElementsReducer = /* @__PURE__ */ _interop_require_wildcard(
				...
				__toCommonJS(IX2ElementsReducer_exports))
			);
			var _IX2VanillaPlugins = /* @__PURE__ */ _interop_require_wildcard(
				(init_IX2VanillaPlugins(),
				__toCommonJS(IX2VanillaPlugins_exports))
			);
	...
	var init_IX2InstancesReducer = __esm({
		'packages/systems/ix2/engine/reducers/IX2InstancesReducer.ts'() {
			...
			continuousInstance = (state, action) => {
				...
				const positionDiff = optimizeFloat2(
				...
				for (let i = 0, { length } = actionGroups; i < length; i++) {
					...
					if (keyframePosition >= keyframe) {
						...
						const hasNextItem =
							nextGroup && keyframePosition !== keyframe;
						toActionItem = hasNextItem
							? nextGroup.actionItems[0]
							: null;
						if (hasNextItem) {
							positionOffset = keyframe / 100;
							positionRange =
								(nextGroup.keyframe - keyframe) / 100;
						}
					}
				}
				const current = {};
				if (fromActionItem && !toActionItem) {
					for (
						let i = 0, { length } = destinationKeys;
						i < length;
						i++
					) {
					...
					const localPosition =
						(position - positionOffset) / positionRange;
				...
				if (active && origin) {
					...
					if (verbose) {
						...
						const verbosePosition = optimizeFloat2(
							Math.min(
								Math.max(0, verboseDelta / verboseDuration),
								1
							)
						);
	...
	function getQuerySelector({ id, selector }) {
		if (id) {
			...
			if (id.indexOf(IX2_ID_DELIMITER2) !== -1) {
				...
				nodeId = pair[1];
				if (
					pageId !== document.documentElement.getAttribute(tmm_PAGE2)
				) {
					return null;
				}
			}
			return `[data-tmm-id="${nodeId}"], [data-tmm-id^="${nodeId}_instance"]`;
		}
		return selector;
	}
			...
			function createFlow(fromRight) {
				return flatRest(function (funcs) {
					...
					while (++index < length) {
						...
						var funcName = getFuncName(func),
							data =
								funcName == 'wrapper' ? getData(func) : void 0;
						if (
							data &&
							isLaziable(data[0]) &&
							data[1] ==
	...
	var init_IX2VanillaEvents = __esm({
		'packages/systems/ix2/engine/logic/IX2VanillaEvents.ts'() {
			...
			withFilter = (filter, handler) => (options, state) =>
				filter(options, state) === true
					? handler(options, state)
					: state;
			baseActionGroupOptions = {
			...
			whenComponentActiveChange = // @ts-expect-error - TS7006 - Parameter 'handler' implicitly has an 'any' type.
				(handler) => (options, oldState) => {
					...
					const isActive =
						[COMPONENT_ACTIVE, COMPONENT_INACTIVE].indexOf(type) !==
						-1
			...
			whenScrollDirectionChange = // @ts-expect-error - TS7006 - Parameter 'handler' implicitly has an 'any' type.
					...
					(options, oldState = {}) => {
						...
						const scrollTopPadding =
							(isPX
								? scrollOffsetValue
								: (innerHeight * (scrollOffsetValue || 0)) /
								  100) / scrollHeightBounds;
						let scrollingDown;
			...
			getComponentInactiveOptions = (
				allowNestedChildrenEvents = true
			) => ({
				...baseActivityActionGroupOptions,
				handler: withFilter(
					allowNestedChildrenEvents ? isOrContainsElement : isElement,
			...
			IX2VanillaEvents_default = {
					...
					handler: (
						...
						switch (basedOn) {
							case EventBasedOn.VIEWPORT: {
								value = isXAxis
									? Math.min(clientX, window.innerWidth) /
									  window.innerWidth
									: Math.min(clientY, window.innerHeight) /
									  window.innerHeight;
								break;
							}
							...
							default: {
								...
								const isMouseEvent =
									nativeEvent.type.indexOf('mouse') === 0;
								if (
									isMouseEvent &&
									isOrContainsElement({
										element,
						...
						if (
							...
							store.dispatch(
								parameterChanged(namespacedParameterId, value)
							);
						}
						return {
							elementHovered,
				...
				[PAGE_SCROLL]: {
					...
					handler: ({ store, eventConfig }) => {
						const { continuousParameterGroupId, reverse } =
							eventConfig;
				...
				[SCROLLING_IN_VIEW]: {
					...
					handler: (
						...
						if (basedOn === EventBasedOn.VIEWPORT) {
							...
							let offsetStartPerc =
								(addStartOffset ? addOffsetValue : 0) / 100;
							let offsetEndPerc =
								(addEndOffset ? endOffsetValue : 0) / 100;
							offsetStartPerc = startsEntering
								? offsetStartPerc
								: 1 - offsetStartPerc;
							offsetEndPerc = startsExiting
								? offsetEndPerc
								: 1 - offsetEndPerc;
							const offsetElementTop =
								elementRect.top +
								Math.min(
									elementRect.height * offsetStartPerc,
									visibleHeight
								);
							...
							const offsetHeight =
								offsetElementBottom - offsetElementTop;
							...
							const fixedScrollPerc =
								fixedScrollTop / fixedScrollHeight;
				...
				[PAGE_START]: {
					...
					handler: withFilter(
						isElement,
						whenPageLoadStart(actionGroupCreator)
					),
				},
			};
		},
			...
			store.dispatch(
				sessionInitialized({
					hasBoundaryNodes: Boolean(
						document.querySelector(BOUNDARY_SELECTOR2)
					),
					reducedMotion:
	...
	function createGroupInstances({
		...
		continuousActionGroups.forEach((actionGroup) => {
			...
			actionItems.forEach((actionItem) => {
				...
				const elementRoot = target.boundaryMode
					? eventElementRoot
					: null;
				const key =
					stringifyTarget2(target) + COLON_DELIMITER4 + actionTypeId;
			...
			if (
				...
				configs.forEach((eventConfig) => {
					...
					elements.forEach((eventTarget, index) => {
						const eventStateKey =
							eventId + COLON_DELIMITER4 + index;
				...
				(element, eventId, eventStateKey) => {
					...
					const handleEventWithConfig = (eventConfig = {}) => {
						...
						if (!shallowEqual2(newState, oldState)) {
							store.dispatch(
								eventStateChanged(eventStateKey, newState)
							);
						}
					};
					if (
		...
		const addListeners = ({
			...
			types
				...
				.forEach((type) => {
					...
					store.dispatch(
						eventListenerAdded(target, [type, handlerFunc])
					);
				});
		};
	...
	function renderInitialGroup({ store, actionListId, eventId }) {
		...
		if (actionList && actionList.useFirstGroupAsInitialState) {
			...
			if (
				!shouldAllowMediaQuery2(mediaQueries, ixSession2.mediaQueryKey)
			) {
				return;
			}
			initialStateItems.forEach((actionItem) => {
				...
				itemElements.forEach((element) => {
					const pluginInstance = shouldUsePlugin
						? createPluginInstance2(actionTypeId)(
								element,
								actionItem
						  )
						: null;
	...
	function startActionGroup({
		...
		const isFirstGroup =
			groupIndex === 0 ||
			(groupIndex === 1 && useFirstGroupAsInitialState);
		const instanceDelay =
	...
	function createInstance(options) {
		...
		const skipMotion =
			...
			ixSession2.reducedMotion &&
			ReducedMotionTypes[actionItem.actionTypeId];
		let skipToValue;
	...
	var require_engine = __commonJS({
		'packages/systems/ix2/engine/index.js'(exports) {
			...
			var _IX2VanillaEngine =
				(init_IX2VanillaEngine(),
				__toCommonJS(IX2VanillaEngine_exports));
			var _IX2EngineActions = /* @__PURE__ */ _interop_require_wildcard(
				(init_IX2EngineActions(),
				__toCommonJS(IX2EngineActions_exports))
			);
	...
	// packages/shared/render/plugins/BaseSiteModules/tmm-ix2.js
	var require_tmm_ix2 = __commonJS({
		'packages/shared/render/plugins/BaseSiteModules/tmm-ix2.js'(
			exports,
			module
		) {
	...
	// packages/shared/render/plugins/BaseSiteModules/tmm-links.js
	var require_tmm_links = __commonJS({
		'packages/shared/render/plugins/BaseSiteModules/tmm-links.js'(
			exports,
			module
		) {
			...
			tmm.define(
				...
				(module.exports = function ($, _) {
					...
					function init() {
						designer = inApp && tmm.env('design');
						slug = tmm.env('slug') || location.pathname || '';
						tmm.scroll.off(scroll);
						anchors = [];
	...
	// packages/shared/render/plugins/BaseSiteModules/tmm-scroll.js
	var require_tmm_scroll = __commonJS({
		'packages/shared/render/plugins/BaseSiteModules/tmm-scroll.js'(
			exports,
			module
		) {
				...
				(module.exports = function ($) {
					...
					function reducedMotionEnabled() {
						return (
							...
							) === 'none' || reducedMotionMediaQuery.matches
						);
					}
					function setFocusable($el, action) {
						...
						switch (action) {
							case 'add':
								...
								break;
							case 'remove':
								initialTabindex = $el.attr(
									'data-tmm-tabindex-swap'
								);
								if (initialTabindex) {
									$el.attr('tabindex', initialTabindex);
									$el.removeAttr('data-tmm-tabindex-swap');
								} else {
					...
					function validateScroll(evt) {
						...
						if (
							// Bail if in Designer
							tmm.env('design') || // Ignore links being used by jQuery mobile
							(window.$.mobile &&
								/(?:^|\s)ui-link(?:$|\s)/.test(
									target.className
								))
						...
						var hash = linksToCurrentPage(target)
							? target.hash
							: '';
						if (hash === '') return;
						var $el = $(hash);
					...
					function updateHistory(hash) {
						if (
							...
							!(tmm.env.chrome && loc.protocol === 'file:')
						) {
					...
					function scroll($targetEl, cb) {
						...
						if (start === end) return;
						var duration = calculateScrollDuration(
							$targetEl,
							start,
							end
						);
						...
						var step = function () {
							...
							window.scroll(
								0,
								getY(start, end, elapsed, duration)
							);
							if (elapsed <= duration) {
								animate(step);
					...
					function calculateScrollDuration($targetEl, start, end) {
						...
						$body.add($targetEl).each(function (_, el) {
							var time = parseFloat(
								el.getAttribute('data-scroll-time')
							);
						...
						return (
							(472.143 * Math.log(Math.abs(start - end) + 125) -
								2e3) *
							mult
						);
					}
					function getY(start, end, elapsed, duration) {
						...
						return (
							start + (end - start) * ease2(elapsed / duration)
						);
					}
					function ease2(t) {
						return t < 0.5
					...
					function ready() {
						var { tmm_CLICK_EMPTY, tmm_CLICK_SCROLL } = NS_EVENTS;
						$doc.on(
							tmm_CLICK_SCROLL,
							localHrefSelector,
							validateScroll
	...
	// packages/shared/render/plugins/BaseSiteModules/tmm-touch.js
	var require_tmm_touch = __commonJS({
		'packages/shared/render/plugins/BaseSiteModules/tmm-touch.js'(
			exports,
			module
		) {
	...
	// packages/shared/render/plugins/Dropdown/tmm-dropdown.js
	var require_tmm_dropdown = __commonJS({
		'packages/shared/render/plugins/Dropdown/tmm-dropdown.js'(
			exports,
			module
		) {
			'use strict';
			var tmm = require_tmm_lib();
			var IXEvents = require_tmm_ix2_events();
			var KEY_CODES = {
				ARROW_LEFT: 37,
				ARROW_UP: 38,
				ARROW_RIGHT: 39,
			...
			tmm.define(
				...
				(module.exports = function ($, _) {
					...
					var inDesigner;
					var touch = tmm.env.touch;
					var namespace = '.tmm-dropdown';
					var openStateClassName = 'tmm--open';
					...
					var mouseUpEvent =
						(touch ? 'click' : 'mouseup') + namespace;
					var closeEvent = 'tmm-close' + namespace;
					...
					function init() {
						inDesigner = inApp && tmm.env('design');
					...
					function build(i, el) {
						...
						data.toggle = data.el.children('.tmm-dropdown-toggle');
						data.list = data.el.children('.tmm-dropdown-list');
						data.links = data.list.find(
							'a:not(.tmm-dropdown .tmm-dropdown a)'
						);
						...
						if (!toggleId) {
							toggleId = 'tmm-dropdown-toggle-' + i;
						}
						if (!listId) {
							listId = 'tmm-dropdown-list-' + i;
						}
						...
						data.links.each(function (idx, link) {
							...
							if (
								INTERNAL_PAGE_LINK_HASHES_PATTERN.test(
									link.hash
								)
							) {
								link.addEventListener(
									'click',
									close.bind(null, data)
								);
							}
						});
						...
						if (!inDesigner) {
							...
							if (data.config.hover) {
								data.toggle.on(
									mouseEnterEvent,
									makeMouseEnterHandler(data)
								);
							}
							data.el.on(closeEvent, initialToggler);
							data.el.on(
								keydownEvent,
								makeDropdownKeydownHandler(data)
							);
							data.el.on(
								focusOutEvent,
								makeDropdownFocusOutHandler(data)
							);
							data.toggle.on(mouseUpEvent, initialToggler);
							data.toggle.on(
								keydownEvent,
								makeToggleKeydownHandler(data)
							);
							data.nav = data.el.closest('.tmm-nav');
					...
					function configure(data) {
						...
						data.config = {
							hover:
								data.el.attr('data-hover') === 'true' && !touch,
					...
					function open(data) {
						...
						tmm.redraw.up();
						data.manageZ &&
							data.el.css('z-index', defaultZIndex + 1);
						var isEditor = tmm.env('editor');
						if (!inDesigner) {
					...
					function close(data, { immediate, forceClose } = {}) {
						...
						data.delayId = window.setTimeout(
							data.complete,
							config.delay
						);
					}
					...
					function outside(data) {
						...
						return debounce(function (evt) {
							...
							if (
								$target.closest('.tmm-dropdown-toggle').length
							) {
								return;
							}
							...
							if (isEventOutsideDropdowns) {
								if (isEditor) {
									...
									var isEventOnHoverControls =
										$target.parents(
											'.tmm-editor-bem-EditorHoverControls'
										).length;
									if (
										isEventOnDetachedSvg ||
						...
						return debounce(function (evt) {
							...
							if (isEventOutsideDropdowns) {
								...
								var isEventOnHoverToolbar = $target.parents(
									'.tmm-editor-bem-RTToolbar'
								).length;
								var $editorOverlay = $(
									'.tmm-editor-bem-EditorOverlay'
								);
								var isDropdownInEdition =
									$editorOverlay.find(
										'.tmm-editor-edit-outline'
									).length ||
									$editorOverlay.find(
										'.tmm-editor-bem-RTToolbar'
									).length;
					...
					function makeDropdownKeydownHandler(data) {
						return function (evt) {
							...
							data.selectedIdx = data.links.index(
	...
	// packages/shared/render/plugins/Form/tmm-forms.js
	var require_tmm_forms = __commonJS({
		'packages/shared/render/plugins/Form/tmm-forms.js'(exports, module) {
			'use strict';
			var tmm = require_tmm_lib();
			...
			tmm.define(
				...
				(module.exports = function ($, _) {
					...
					function init() {
						siteId = $('html').attr('data-tmm-site');
						formUrl = 'https://tmm.com/api/v1/form/' + siteId;
						if (retro && formUrl.indexOf('https://tmm.com') >= 0) {
					...
					function build(i, el) {
						...
						reset(data);
						var wrap = $el.closest('div.tmm-form');
						data.done = wrap.find('> .tmm-form-done');
						data.fail = wrap.find('> .tmm-form-fail');
						data.fileUploads = wrap.find('.tmm-file-upload');
						...
						if (siteId) {
							data.handler = true
								? exportedSubmittmm
								: (() => {
										const hostedSubmitHandler = null
											.default;
					...
					function addListeners() {
						...
						const CHECKBOX_CLASS_NAME = '.tmm-checkbox-input';
						const RADIO_INPUT_CLASS_NAME = '.tmm-radio-input';
						const CHECKED_CLASS = 'tmm--redirected-checked';
						const FOCUSED_CLASS = 'tmm--redirected-focus';
						const FOCUSED_VISIBLE_CLASS =
							'tmm--redirected-focus-visible';
						const focusVisibleSelectors =
							':focus-visible, [data-tmm-focus-visible]';
						...
						$doc.on(
							...
							(evt) => {
								...
								if (!$target.hasClass('tmm-radio-input')) {
									$target
										.siblings(RADIO_INPUT_CLASS_NAME)
					...
					function reset(data) {
						var btn = (data.btn = data.form.find(
							':input[type="submit"]'
						));
					...
					function findFields(form, result) {
						...
						form.find(
							...
							result[name] = value;
							status =
								status || getStatus(field, type, name, value);
						});
						return status;
					}
					...
					function exportedSubmittmm(data) {
						preventDefault(data);
						afterSubmit(data);
					}
					function submitMailChimp(data) {
						...
						var url =
							data.action.replace('/post?', '/post-json?') +
							'&c=?';
						var userId = url.indexOf('u=') + 2;
						userId = url.substring(
							userId,
							url.indexOf('&', userId)
						);
						var listId = url.indexOf('id=') + 3;
						listId = url.substring(
							listId,
							url.indexOf('&', listId)
						);
					...
					function initFileUpload(i, form) {
						...
						var $defaultWrap = $el.find(
							'> .tmm-file-upload-default'
						);
						var $uploadingWrap = $el.find(
							'> .tmm-file-upload-uploading'
						);
						var $successWrap = $el.find(
							'> .tmm-file-upload-success'
						);
						var $errorWrap = $el.find('> .tmm-file-upload-error');
						var $input = $defaultWrap.find(
							'.tmm-file-upload-input'
						);
						var $label = $defaultWrap.find(
							'.tmm-file-upload-label'
						);
						var $labelChildren = $label.children();
						var $errorMsgEl = $errorWrap.find(
							'.tmm-file-upload-error-msg'
						);
						var $fileEl = $successWrap.find(
							'.tmm-file-upload-file'
						);
						var $removeEl = $successWrap.find(
							'.tmm-file-remove-link'
						);
						var $fileNameEl = $fileEl.find(
							'.tmm-file-upload-file-name'
						);
						var sizeErrMsg = $errorMsgEl.attr(
							'data-tmm-size-error'
						);
						var typeErrMsg = $errorMsgEl.attr(
							'data-tmm-type-error'
						);
						var genericErrMsg = $errorMsgEl.attr(
							'data-tmm-generic-error'
						);
						if (!inApp) {
						...
						function parseError(err) {
							var errorMsg =
								err.responseJSON && err.responseJSON.msg;
	...
	// packages/shared/render/plugins/Lightbox/tmm-lightbox.js
	var require_tmm_lightbox = __commonJS({
		'packages/shared/render/plugins/Lightbox/tmm-lightbox.js'(
			exports,
			module
		) {
			'use strict';
			var tmm = require_tmm_lib();
			var CONDITION_INVISIBLE_CLASS = 'tmm-condition-invisible';
			...
			function isConditionallyHidden(item) {
				return Boolean(
					item.$el && item.$el.closest(CONDVIS_SELECTOR).length
				);
			}
			...
			function createLightbox(window1, document1, $, container) {
						...
						items.forEach(function (item, idx) {
							...
							loadImage(
								...
								function ($image) {
									...
									$thumbnail.append(
										addClass($image, 'thumbnail-image')
									);
								}
							);
						});
				...
				lightbox.build = function () {
					...
					$refs.container = dom('container').append(
						$refs.content,
						$refs.strip
					);
					$refs.lightbox = dom('backdrop hide').append(
						$refs.container
					);
				...
				lightbox.show = function (index) {
					...
					spinner.show();
					var url =
						(item.html && svgDataUri(item.width, item.height)) ||
						item.url;
					loadImage(url, function ($image) {
						...
						if (item.caption) {
							$figure.append(
								dom('caption', 'figcaption').text(item.caption)
							);
						}
						...
						function transitionToNewView() {
							...
							const shouldHideLeftArrow =
								shouldSetArrowLeftInactive(index, items);
							toggleClass(
								$refs.arrowLeft,
								'inactive',
								shouldHideLeftArrow
							...
							if (
								shouldHideLeftArrow &&
								$refs.arrowLeft.is(':focus')
							) {
								$refs.arrowRight.focus();
							}
							const shouldHideRightArrow =
								shouldSetArrowRightInactive(index, items);
							toggleClass(
								$refs.arrowRight,
								'inactive',
					...
					if (resetVisibilityState.length === 0) {
						$('body')
							...
							.each(function () {
								if (
									$(this).hasClass('tmm-lightbox-backdrop') ||
									$(this).is('script')
								) {
									return;
								}
				...
				function keyHandler(event) {
					...
					if (
						keyCode === 27 ||
						checkForFocusTrigger(keyCode, 'close')
					) {
						lightbox.hide();
					} else if (
						keyCode === 37 ||
						checkForFocusTrigger(keyCode, 'left')
					) {
						lightbox.prev();
					} else if (
						keyCode === 39 ||
						checkForFocusTrigger(keyCode, 'right')
					) {
						lightbox.next();
				...
				function hideLightbox() {
					if ($refs) {
						...
						resetVisibilityState.forEach(function (
							visibilityState
						) {
				...
				function maybeScroll($item) {
					...
					var stripScrollLeftMax =
						stripElement.scrollWidth - stripWidth;
					var newScrollLeft;
					if (itemLeft < stripScrollLeft) {
						newScrollLeft = Math.max(
							0,
							itemLeft + itemWidth - stripWidth
						);
					} else if (
						itemLeft + itemWidth >
						stripWidth + stripScrollLeft
					) {
				...
				function dom(className, tag) {
					return addClass(
						$(document1.createElement(tag || 'div')),
						className
					);
				}
			...
			tmm.define(
				...
				(module.exports = function ($) {
					...
					function configure(data) {
						var json = data.el.children('.tmm-json').html();
						var groupName;
						var groupItems;
						if (!json) {
							data.items = [];
					...
					function clickHandler(data) {
						return function () {
							data.items.length &&
								lightbox(data.items, data.index || 0);
						};
					}
	...
	// packages/shared/render/plugins/Navbar/tmm-navbar.js
	var require_tmm_navbar = __commonJS({
		'packages/shared/render/plugins/Navbar/tmm-navbar.js'(exports, module) {
			'use strict';
			var tmm = require_tmm_lib();
			var IXEvents = require_tmm_ix2_events();
			var KEY_CODES = {
			...
			tmm.define(
				...
				(module.exports = function ($, _) {
					...
					var inApp = tmm.env();
					var overlay =
						'<div class="tmm-nav-overlay" data-tmm-ignore />';
					var namespace = '.tmm-nav';
					var navbarOpenedButton = 'tmm--open';
					var navbarOpenedDropdown = 'tmm--nav-dropdown-open';
					var navbarOpenedDropdownToggle =
						'tmm--nav-dropdown-toggle-open';
					var navbarOpenedDropdownList =
						'tmm--nav-dropdown-list-open';
					var navbarOpenedLink = 'tmm--nav-link-open';
					var ix = IXEvents.triggers;
					var menuSibling = $();
					api.ready = api.design = api.preview = init;
					...
					function removeListeners() {
						tmm.resize.off(resizeAll);
					}
					function addListeners() {
						tmm.resize.on(resizeAll);
					}
					function resizeAll() {
						$navbars.each(resize);
					}
					function build(i, el) {
						...
						data.menu = $el.find('.tmm-nav-menu');
						data.links = data.menu.find('.tmm-nav-link');
						data.dropdowns = data.menu.find('.tmm-dropdown');
						data.dropdownToggle = data.menu.find(
							'.tmm-dropdown-toggle'
						);
						data.dropdownList =
							data.menu.find('.tmm-dropdown-list');
						data.button = $el.find('.tmm-nav-button');
						data.container = $el.find('.tmm-container');
						data.overlayContainerId = 'tmm-nav-overlay-' + i;
						data.outside = outside(data);
						var navBrandLink = $el.find('.tmm-nav-brand');
						if (
							navBrandLink &&
					...
					function configure(data) {
						...
						config.duration =
							duration != null ? Number(duration) : 400;
					...
					function navigate(data) {
						return function (evt) {
							...
							if (!tmm.validClick(evt.currentTarget)) {
								evt.preventDefault();
								return;
							}
					...
					function outside(data) {
						...
						return function (evt) {
							...
							if (
								inEditor &&
								$target.closest('.tmm-editor-bem-EditorOverlay')
									.length
							) {
								return;
							}
					...
					var outsideDebounced = debounce(function (data, $target) {
						...
						var menu = $target.closest('.tmm-nav-menu');
					...
					function open(data, immediate) {
						...
						data.dropdownToggle.addClass(
							navbarOpenedDropdownToggle
						);
					...
					function setOverlayHeight(data) {
						var config = data.config;
						var bodyHeight = config.docHeight
							? $doc.height()
							: $body.height();
						if (config.animOver) {
					...
					function close(data, immediate) {
						...
						function complete() {
							...
							data.dropdownToggle.removeClass(
								navbarOpenedDropdownToggle
							);
							data.dropdownList.removeClass(
								navbarOpenedDropdownList
							);
							if (
								data.overlay &&
							...
							data.el.triggerHandler('tmm-close');
							data.button.attr('aria-expanded', 'false');
						}
					}
					return api;
	...
	// packages/shared/render/plugins/Slider/tmm-slider.js
	var require_tmm_slider = __commonJS({
		'packages/shared/render/plugins/Slider/tmm-slider.js'(exports, module) {
			'use strict';
			var tmm = require_tmm_lib();
			var IXEvents = require_tmm_ix2_events();
			var KEY_CODES = {
			...
			tmm.define(
				...
				(module.exports = function ($, _) {
					...
					var designer;
					var inApp = tmm.env();
					var namespace = '.tmm-slider';
					var dot = '<div class="tmm-slider-dot" data-tmm-ignore />';
					var ariaLiveLabelHtml =
						'<div aria-live="off" aria-atomic="true" class="tmm-slider-aria-label" data-tmm-ignore />';
					var forceShow = 'tmm-slider-force-show';
					var ix = IXEvents.triggers;
					var fallback;
					var inRedraw = false;
					api.ready = function () {
						designer = tmm.env('design');
						init();
					};
					api.design = function () {
						designer = true;
					...
					function removeListeners() {
						tmm.resize.off(renderAll);
						tmm.redraw.off(api.redraw);
					}
					function addListeners() {
						tmm.resize.on(renderAll);
						tmm.redraw.on(api.redraw);
					}
					function renderAll() {
					...
					function build(i, el) {
						...
						data.mask = $el.children('.tmm-slider-mask');
						data.left = $el.children('.tmm-slider-arrow-left');
						data.right = $el.children('.tmm-slider-arrow-right');
						data.nav = $el.children('.tmm-slider-nav');
						data.slides = data.mask.children('.tmm-slide');
						...
						if (!slideViewId) {
							slideViewId = 'tmm-slider-mask-' + i;
							data.mask.attr('id', slideViewId);
						}
						if (!designer && !data.ariaLiveLabel) {
							data.ariaLiveLabel = $(ariaLiveLabelHtml).appendTo(
								data.mask
							);
						}
						...
						if (designer) {
							...
							data.left.on(
								'click' + namespace,
								previousFunction(data)
							);
							data.right.on('click' + namespace, next(data));
							data.left.on(
								'keydown' + namespace,
								keyboardSlideButtonsFunction(
									data,
									previousFunction
								)
							);
					...
					function configure(data) {
						...
						config.crossOver = 0;
						config.animation =
							data.el.attr('data-animation') || 'slide';
						...
						var duration = data.el.attr('data-duration');
						config.duration =
							duration != null ? parseInt(duration, 10) : 500;
						...
						if (isAttrTrue(data.el.attr('data-autoplay'))) {
							...
							config.delay =
								parseInt(data.el.attr('data-delay'), 10) || 2e3;
					...
					function keyboardSlideButtonsFunction(
						data,
						directionFunction
					) {
						return function (evt) {
						...
						if (animation === 'over') {
							...
							tram(targets)
								.set({
									...
									x:
										offsetX +
										anchors[data.index].width * vector,
								})
								.add(slideRule)
								.start({
									x: offsetX,
					...
					function buildNav(data) {
						...
						for (var i = 0, len = data.pages; i < len; i++) {
							...
							$dot.attr(
								'aria-label',
								'Show slide ' + (i + 1) + ' of ' + len
							)
								.attr('aria-pressed', 'false')
								.attr('role', 'button')
								.attr('tabindex', '-1');
							if (data.nav.hasClass('tmm-num')) {
								$dot.text(i + 1);
							}
							if (spacing != null) {
								$dot.css({
	...
	// packages/shared/render/plugins/Tabs/tmm-tabs.js
	var require_tmm_tabs = __commonJS({
		'packages/shared/render/plugins/Tabs/tmm-tabs.js'(exports, module) {
			'use strict';
			var tmm = require_tmm_lib();
			var IXEvents = require_tmm_ix2_events();
			tmm.define(
				...
				(module.exports = function ($) {
					...
					var inApp = env();
					var tabAttr = 'data-tmm-tab';
					var paneAttr = 'data-tmm-pane';
					var namespace = '.tmm-tabs';
					var linkCurrent = 'tmm--current';
					var tabActive = 'tmm--tab-active';
					var ix = IXEvents.triggers;
					var inRedraw = false;
					api.ready = api.design = api.preview = init;
					api.redraw = function () {
					...
					function init() {
						design = inApp && tmm.env('design');
						$tabs = $doc.find(namespace);
						if (!$tabs.length) {
							return;
						}
						$tabs.each(build);
						if (tmm.env('preview') && !inRedraw) {
							$tabs.each(resetIX);
						}
						removeListeners();
						addListeners();
					...
					function build(i, el) {
						...
						data.menu = $el.children('.tmm-tab-menu');
						data.links = data.menu.children('.tmm-tab-link');
						data.content = $el.children('.tmm-tab-content');
						data.panes = data.content.children('.tmm-tab-pane');
						data.el.off(namespace);
					...
					function configure(data) {
						...
						config.easing = data.el.attr('data-easing') || 'ease';
						var intro = parseInt(
							data.el.attr('data-duration-in'),
							10
						);
						intro = config.intro = intro === intro ? intro : 0;
						var outro = parseInt(
							data.el.attr('data-duration-out'),
							10
						);
					...
					function changeTab(data, options) {
						...
						if ($previous.length && config.outro) {
							...
							tram($previous)
								.add(
									'opacity ' + config.outro + 'ms ' + easing,
									{
										fallback: safari,
									}
								)
	...
	// <stdin>
	require_tmm_brand();
	require_tmm_focus_visible();
	require_tmm_focus();
	require_tmm_ix2_events();
	require_tmm_ix2();
	require_tmm_links();
	require_tmm_scroll();
	require_tmm_touch();
	require_tmm_dropdown();
	require_tmm_forms();
	require_tmm_lightbox();
	require_tmm_navbar();
	require_tmm_slider();
	require_tmm_tabs();
})();
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'df32ed72-b042-0e2f-1afd-b9d68f8711d2',
									],
								},
								widthValue: 100,
								heightValue: 0,
						...
						},
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'df32ed72-b042-0e2f-1afd-b9d68f8711cd',
									],
								},
								zValue: 45,
								xUnit: 'DEG',
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									useEventTarget: 'SIBLINGS',
									selector:
										'.tmm-product-options1-accordion-details',
									selectorGuids: [
										'63a35aae-04d8-662e-40ea-1797e6b41346',
									],
								},
								heightValue: 0,
								widthUnit: 'PX',
						...
						},
						{
							...
							config: {
								...
								target: {
									useEventTarget: 'CHILDREN',
									selector:
										'.tmm-product-options1-accordion-icon',
									selectorGuids: [
										'63a35aae-04d8-662e-40ea-1797e6b4132a',
									],
								},
								zValue: 180,
								xUnit: 'DEG',
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'f23d937b-95b2-589a-efcb-01b62a799fe0',
									],
								},
								widthValue: 100,
								heightValue: 0,
						...
						},
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'f23d937b-95b2-589a-efcb-01b62a799fe4',
									],
								},
								zValue: 0,
								xUnit: 'DEG',
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'd48f1e8e-9e87-9624-893f-7e5e0bcec2c0',
									],
								},
								yValue: 1,
								locked: false,
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'e6981d25-49a2-d637-c264-8138653dea55',
									],
								},
								value: 'none',
							},
						},
						{
							...
							config: {
								...
								target: {
									selector:
										'.tmm-product-options2-content_wrap',
									selectorGuids: [
										'e6981d25-49a2-d637-c264-8138653dea58',
									],
								},
								yValue: 100,
								xUnit: 'PX',
						...
						},
						{
							...
							config: {
								...
								target: {
									selector:
										'.tmm-product-options2-background-overlay',
									selectorGuids: [
										'e6981d25-49a2-d637-c264-8138653dea5a',
									],
								},
								value: 0,
								unit: '',
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'298bbb8a-b496-e03d-5452-dbb5682b8dce',
									],
								},
								heightValue: 0,
								widthUnit: 'PX',
						...
						},
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'298bbb8a-b496-e03d-5452-dbb5682b8dc9',
									],
								},
								zValue: 0,
								xUnit: 'DEG',
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									useEventTarget: 'SIBLINGS',
									selector:
										'.tmm-product-options3-accordion-details',
									selectorGuids: [
										'b23f7e16-5156-b5c6-cfa9-be621edf2a4e',
									],
								},
								heightValue: 0,
								widthUnit: 'PX',
						...
						},
						{
							...
							config: {
								...
								target: {
									useEventTarget: 'CHILDREN',
									selector:
										'.tmm-product-options3-accordion-icon',
									selectorGuids: [
										'b23f7e16-5156-b5c6-cfa9-be621edf2a4a',
									],
								},
								zValue: 45,
								xUnit: 'DEG',
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									useEventTarget: 'CHILDREN',
									selector:
										'.tmm-sticky-timeline-progress-bar_level',
									selectorGuids: [
										'1f37fd63-983c-914b-0e8a-5cedc764d7ba',
									],
								},
								widthValue: 0,
								widthUnit: '%',
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'f8aeeea0-2fbf-452f-26e1-b2064a682d72',
									],
								},
								xValue: 0,
								xUnit: '%',
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'f8aeeea0-2fbf-452f-26e1-b2064a682d74',
									],
								},
								yValue: 0,
								xUnit: 'PX',
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'e73d2ae5-3ca9-f77c-e6ca-f4812a33ea54',
									],
								},
								xValue: null,
								yValue: 0,
						...
						},
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'e73d2ae5-3ca9-f77c-e6ca-f4812a33ea58',
									],
								},
								xValue: null,
								yValue: 0,
						...
						},
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'e73d2ae5-3ca9-f77c-e6ca-f4812a33ea51',
									],
								},
								value: 1,
								unit: '',
						...
						},
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'f21c004b-7a74-ff28-bb8e-2fda7e111aa1',
									],
								},
								xValue: null,
								yValue: 0,
						...
						},
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'e73d2ae5-3ca9-f77c-e6ca-f4812a33ea57',
									],
								},
								xValue: null,
								yValue: 0,
							...
							actionItems: [
								{
									...
									config: {
										...
										target: {
											...
											selectorGuids: [
												'f21c004b-7a74-ff28-bb8e-2fda7e111a9f',
											],
										},
										yValue: -10,
										xUnit: 'PX',
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'f21c004b-7a74-ff28-bb8e-2fda7e111a9d',
									],
								},
								xValue: -100,
								xUnit: '%',
					...
					actionItems: [
						{
							...
							config: {
								...
								target: {
									...
									selectorGuids: [
										'b822e631-3e24-3b3b-070e-1f886e84e8de',
									],
								},
								xValue: -100,
								xUnit: '%',
